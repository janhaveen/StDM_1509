...
{
	$("#HomeText").html("Request For Purchase List");
	
	getRequestForPurchaseList();
	
});


//Product NPD list
function getRequestForPurchaseList() 
{
		//spinnerOn();
	    ...
	    var actionStr = "",
	        str = "?1=1";
	    var i = 0;
	    var url = "getRequestForPurchaseList";
	    if (window.location.href.indexOf("showRequestForPurchase") >= 0) 
	    {
	        actionStr = '<i class="edit_me fa fa-check fa-2x" aria-hidden="true"></i> ';
	      
	    } 
	
		$('#requestForPurchaseListTable').DataTable().destroy();
	
		$('#requestForPurchaseListTable thead #filterrow th').each( function () {
	    	var title = $(this).text();
	        $(this).html( '<input type="text" style="font-size: 80%;padding:1%;text-align:center" placeholder="'+title+'" />' );
	    });
		
    	$("#requestForPurchaseListTable thead input").on('keyup change', function () {
    	    table.column($(this).parent().index() + ':visible')
    	        .search(this.value)
    	        .draw();
    	});
    	
    	 var table=$('#requestForPurchaseListTable').DataTable({ 
    		// dom: 'Bfrtip',
             buttons: [
                 {
    	 ...
    	 $('#requestForPurchaseListTable tbody').off('click');
    	 
    	 $('#requestForPurchaseListTable tbody').on( 'click', '.edit_me', function () 
    	 {
    		 var data = table.row( $(this).parents('tr') ).data();
    		 
    		 SwalOfNPDProductSentInfo("Are You Want To Change Status This Product !",  data);
