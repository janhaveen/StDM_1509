$(document).ready(function(){
    ...
    $("#DeleteVendorConfirmed").click(function() {
        ...
        $.ajax({
            type: "GET",
            url: '../../../ModifyVendor?DeleteVendorId=' + DeleteVendorId,
            success: function(data) {
                if (data == 0) {
...
function submitPart(url) {
	...
	console.log($("#newVendorElementForm").serialize());
	$.ajax({
       type: "POST",
       url: url,
       data: $("#newVendorElementForm").serialize()+data,
       success: function(data)
       {
           if (data == 1) {
        	   $("#VendorTableDiv").removeClass("HideThisElement");
