$(document).ready(function(){
	...
	if($("#departmentHeader").val().indexOf("Sales")<0){
		DropDownForEmployee("");
		$("#OpoDiv").removeClass("HideThisElement");
	}
	if($("#for").val() == "newSample")
	{
		$("#action").val("insert");
		$('#ProductDatatables').DataTable().destroy();
		getProductList();ProductClickFn();
...
function formSubmit()
{
	...
	if($("#action").val() == "insert" || $("#action").val() == "updateAccpStatus")
	{
		...
		for (var i = 0; i < rowCount; i++)
		{
			...
			+$("#expectedReceiptDate").val()+"&remarks="+$("#remarks").val()+"&action="+$("#action").val()+"&count="+count+"&salespersonSelected="+$('#SalesPersonR').val();
