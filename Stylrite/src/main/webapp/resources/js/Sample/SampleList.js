...
$('#datatables tbody').on('click', '.select_me', function()
	...
	$(" #SampleFormDiv #SampleFormProductListDiv").removeClass("HideThisElement");
	...
	$("#remarksDiv").html("");	$('#ProductDataBody').html('');

	getRemarksHistory(data.requisitionId,0,1);
	getProducts(data.requisitionId);
	
	$("#backSampleListButton").addClass("HideThisElement");
...
$("#SampleFormProductListDiv").removeClass("HideThisElement");
 ...
 /* document.querySelector('#ProductDataBody').value = "";*/
	...
	$("#backSampleListButton").removeClass("HideThisElement");
 //   document.getElementById("newElementForm").reset();
	$("#sourceText").text(data.source_text);
	$("#creditTimeText").text(data.creditTime);
	$("#creditLimitText").text(data.creditLimit);
...
function getProducts(rowId) 
{
	...
	spinnerOn();
	$.ajax({
		url:"GetProductRequisitionList?requisitionId="+rowId,
		type:"GET",
		success:function(data){
	    	...
	    	for(var i=0; i<data.data.length; i++)
	    	{
	    		/*alert("productid");*/
	    		spinnerOff();
	    		console.log("productrequistionlist",data.data[0]);
	    		console.log("J",data.data[i].productId);
	    		console.log(data.data[i].skuid,"k");
			    var newHtml = '<tr id="productIdRow">'+
				  ...
				  '<td id="chck'+data.data[i].productId+'" class="'+data.data[i].class1+'">'+data.data[i].rqtnItmstatus_text+'</td>'+
				  '<td style="display:none;" id="requitionItemId">'+data.data[i].rowId+'</td>'+
				  '<td style="display:none;">'+data.data[i].status+'</td>'+
				  '<td class="HideThisElement"><input type="text" id="barcode'+data.data[i].productId+'" name="barcode' + data.data[i].productId + '" class="form-control" ></td>'+
				  '</tr>';
...
//Back Button of SampleView.jsp
$("#SubmitButtonBackFromSampleListView").click(function() 
{
	$("#ProductDataBody").empty();
