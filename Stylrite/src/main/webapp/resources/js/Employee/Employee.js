...
function formSubmit(type){ console.log(type);
	...
	if (document.getElementById("firstName").value == "") { 
		...
		var formData = "selectedEmployeeId=" + $('#selectedEmployeeId').val();
    	...
    	$.ajax({
               type: "POST",
               url: url,
               data: $("#newElementForm").serialize()+"&"+formData, // serializes the form's elements.
               success: function(data)
               {
                   if (data == 1) {
                	   document.getElementById("newElementForm").reset();
						...
						if(type=="new"){
							document.querySelector('#errorMsg').innerHTML = "<strong>Error!</strong> Failed to Register New Employee !";
							$(".errorMsg").removeClass("HideThisElement");
						}else{
							document.querySelector('#errorMsg').innerHTML = "<strong>Error!</strong> Failed to Update New Employee !";
							$(".errorMsg").removeClass("HideThisElement");
						}
					}
               }
